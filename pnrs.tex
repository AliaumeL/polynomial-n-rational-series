...
\begin{lemma}
    ...
    and $P = Y^p M$ where $M$ is a \kl{monomial} over $\vec{X}$.
    Then, for all $K \geq 0$,
    for all $\nu \colon \vec{X}Y \topartial \Nat$
    such that $\nu(Y) > K$ or $\nu(Y)$ is undefined,
    $\restr{\Diff{K}{Y^pM}{Y^pM}}{\nu}$
    has \kl{maximal monomials} with
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \quad .
    \end{align*}
    Which proves that the \kl{maximal monomial} of
    $\restr{\Delta_K}{\nu}$ has a coefficient
    that is at least $K$.
\end{proof}
...
\begin{proof}
    ...
    $\nu(Y)$ is undefined, the \kl{maximal monomials} of $\restr{Q_2}{\nu}$
    have \kl{positive coefficients}, and that these coefficients are either $0$
    (the polynomial is constant equal to zero) or at least $K$.
    
    To conclude, it is just a matter of describing how \kl{maximal monomials}
    ...
    and that its (unique) \kl{maximal monomial} has a \kl{positive coefficient}
    that is at least $K$.
    Let us now consider a \kl{maximal monomial} $M_0$
    of $\restr{P}{\nu}$. This \kl{monomial} is obtained
    by summing \kl{monomials} from $\restr{Q_1}{\nu}$ and $\restr{Q_2}{\nu}$.
    Note that, by construction 

    Because $\alpha Y^p M$ is a \kl{maximal monomial} of
    ...
    If a \kl{maximal monomial} of $\restr{\Delta_K}{\nu}$
    is obtained
...
\kl{maximal monomials}. To make the induction formal, let us define
$\Monomials[\vec{X}]$ as the (infinite) set of all possible \kl{monomials} over
$\vec{X}$, ordered by \kl{divisibility}. Remark that this set is isomorphic to
