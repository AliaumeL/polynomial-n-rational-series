...
property using \kl{(noncommutative output) polynomial functions}, allowing us
to use the same formalism in \cref{k-combinatorial:def} and
\cref{ultimately-polynomial:def} and highlight the role of \kl{commutative}
...
non-commutative monomial over the semi- $\Sigma^*$. Almost by
...
is \kl{star-free polyregular}, as described in the following lemma.
...
\begin{corollary}
    ...
    \begin{enumerate}
        \item \label{rat-npoly-0:item} $P$ is a \kl{non-negative} \kl{natural binomial polynomial},
        \item \label{rat-npoly-1:item} $P$ is a \kl{natural binomial polynomial} such that $P(0,\dots,0) \geq 0$,
...
\begin{proof}
    ...
    If $P$ is a \kl{natural binomial polynomial} with $P(0,\dots,0) \geq 0$,
    then it is \kl{non-negative}.

    Proof of \cref{rat-npoly-0:item} $\implies$ \cref{rat-npoly-3:item}.
    If $P$ is a \kl{non-negative} \kl{natural binomial polynomial},
    then for large enough values, it is a \kl{natural binomial function},
    hence computable, and for small values we can hardcode.

    Proof of \cref{rat-npoly-2:item} $\implies$ \cref{rat-npoly-4:item}.
    If $P$ is a \kl{$\Nat$-rational polynomial}, then $\alpha P$ is one too,
    ...
    to conclude that $P$ is actually a \kl{natural binomial polynomial}.
\end{proof}
...
\begin{proof}
    ...
    Let us argue that $f_+$ and $f_-$ are \kl{star-free polyregular functions},
    because they are \kl{ultimately polynomial}. (todo later).
