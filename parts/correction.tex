...
\subsubsection{From $\Rel$ to $\Rat$ and $\Real$}
...
\begin{conjecture}
    ...
    $\restr{P}{\nu}$ is a \kl{natural binomial polynomial} \kl{in base $K$}
    for all partial valuations $\nu \colon \vec{X} \topartial \set{0, \dots, K}$.
...
\kl{integer binomial polynomials} \cite{POLYA1915,CACHA1996}. However, its
...
not enough to ensure that the polynomial is a \kl{$\Nat$-rational polynomial}.

\begin{example}
    The polynomial $\pbinom{X - 2}{2} + 1$ is a \kl{non-negative} \kl{natural binomial polynomial}
    but in the standard basis, it is written 
    $\pbinom{X}{2} - \pbinom{X}{1} + 2$, which has a \kl{negative} coefficient.
\end{example}
...
\begin{corollary}
    ...
    \begin{enumerate}
        ...
        \item \label{int-bin-1:item} $P$ \kl{represented} by a \kl{star-free $\Nat$-polyregular function},
