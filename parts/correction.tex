...
functions that can be computed by \kl{star-free $\Nat$-polyregular
	functions} are going to be suitable combinations of \kl{natural binomial
	functions}. This motivates the following definition of the ``polynomial
counterpart" of a binomial coefficient, $\intro*\pbinom{X - \ell}{k}$
defined as $(X - \ell) \cdots (X - \ell - k) / k!$, that we call a
\intro{binomial monomial}, and introduce \intro{natural binomial
	polynomials} as $\Nat$-linear combinations of products of \kl{binomial
	monomials}, i.e., of the shape: $P(X_1, \dots, X_k) = \sum_{i = 1}^n n_i
...
$\Rel$-linear combinations of products of \kl{binomial monomials}.
...
\begin{proof}
	...
	and $\Rel$-linear combination, we conclude that all \kl{integer binomial polynomials} are
	\kl{integer-valued}.
...
\AP We say that a \kl{natural binomial polynomial} is \intro{in base $K$} if
all its \kl{binomial monomials} are of the form $\pbinom{X - K}{d}$ for some $d
	\in \Nat$. To address the issues raised by the previous examples, we introduce
the notion of \intro{strongly natural binomial polynomials} \kl{in base $K$},
as the polynomials $P \in \Rat[X]$ such that for all partial valuation $\nu
	\colon \topartial \set{0, \dots, K}$, $\restr{P}{\nu}$ is a \kl{natural
	binomial polynomial} \kl{in base $K$}. This leads to the following
characterization.
...
\begin{corollary}
	...
	\begin{enumerate}
		\item \label{rat-npoly-1:item} $P$ is \kl{represented} by a \kl{star-free $\Nat$-polyregular function},
		\item \label{rat-npoly-2:item}
		      $P$ is a \kl{strongly natural binomial polynomial} \kl{in base $K$},
		\item \label{rat-npoly-3:item} $P$ is \kl{represented} by a \kl{$\Nat$-polyregular function},
...
\begin{proof}
	...
	$a^{k}$, which is a \kl{polyregular function} (but is not
	a \kl{star-free polyregular function}). We conclude that $P$ is
	...
	greater than $K$, $P$ coincides with a \kl{natural binomial function},
	which is \kl{represented} by a \kl{star-free $\Nat$-polyregular function}. If some
	input value $X_i$ is set to a number $x_i \leq K$, then one can leverage
	the fact that $\restr{P}{X_i = x_i}$ remains a \kl{strongly natural
		binomial polynomial} \kl{in base $K$}, and we conclude by induction that
	$\restr{P}{X_i = x_i}$ is \kl{represented} by a \kl{star-free $\Nat$-polyregular
		function}. Combining these functions, we obtain a \kl{star-free $\Nat$-polyregular
		function} \kl{representing} $P$.
	...
	remarking that the
	\kl{star-free $\Nat$-polyregular functions} have output in $\Nat$ and are
