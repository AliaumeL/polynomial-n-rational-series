...
We conclude from \cref{lem:correct-covered-2} that $\CoveredPoly[X,Y]$ is a
decidable class of polynomials, hence that Hilbert's Tenth Problem is
decidable for polynomials in two indeterminates with non-negative maximal
monomials.
...
\begin{lemma}[restate=polyrec-integer-strong:lem,label=polyrec-integer-strong:lem]
    ...
    such that $n P \in \Rel[\vec{X}] \cap \CorrectPoly$.
