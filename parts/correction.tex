...
To that end, let us write $\translate{K}$ for the \intro{translation function}
...
\kl{translations operators} do not perfectly commute with partial applications
$\restr{\cdot}{\nu}$.
...
To build some intuition on the proof to come, let us sketch the argument in the
case of a single indeterminate $X$. Given a polynomial $P \in \CorrectPoly$,
the goal is to build a \kl{$\Nat$-polyregular function} $f$ that
\kl{represents} $P$. Note that in this case, the function has unary input and
unary output. We prove the result by induction on the degree $\deg(P)$ of the
polynomial $P$, where the base case is trivial because when $P$ is a constant
function, this constant must be in $\Nat$. For the induction step, we can
assume that $P = \alpha X^n + Q$, where $\deg(Q) < \deg(P)$, $n \geq 1$, and
$\alpha > 0$. On the one hand, $\alpha X^n$ is a \kl{$\Nat$-rational
polynomial}, but on the other hand, $Q$ may not belong to $\CorrectPoly$.
However, there exists $K \in \Nat$ such that $\Diff{K}{X^n} + \translate{K}(Q)$
belongs to $\CorrectPoly$, for instance by considering $K$ to be the sum of
absolute values of coefficients of $P$. Furthermore, $\Diff{K}{X^n} +
\translate{K}(Q)$ has degree smaller than $\deg(P)$, hence is a
\kl{$\Nat$-rational polynomial}. Now, $\translate{K}(P) = \alpha X^n +
\Diff{K}{X^n} + \translate{K}(Q)$ is a \kl{$\Nat$-rational polynomial},
represented by a function $f_1$. Now, for values $k$ of $X$ that are smaller
than $K$, we can compute $P(k) = \restr{P}{X=k}$, which belongs to $\Nat$. It
is possible to build a \kl{$\Nat$-polyregular function} $f_2 \colon \Nat \to
\Nat$ that outputs $P(k)$ if the input is at most $K$, and $f_1(k - K)$
otherwise.
...
When multiple indeterminate are involved, the notion of 
degree must be adapted, and to ensure that the decision procedure
is effective, one must obtain explicit bounds on the
translations that are needed.


\AP To that end, let us now introduce the ordering over which the induction of
...
combinatorial argument of this section. Note that we must be careful
about it. \textbf{TODO: write a nice thing here!}.
...
Let us also state that polynomials in $\CorrectPoly$ are, up to suitable
