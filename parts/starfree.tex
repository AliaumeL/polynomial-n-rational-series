...
\begin{lemma}
    ...
    such that for all $x_1, \dots, x_n \geq \omega$:
    \begin{equation*}
...
\end{theorem}
\begin{proof}
    Let $f$ be a \kl{commutative} \kl{$\Nat$-polyregular function}. If $f \in \NSF \subseteq \ZSF$
    then $f$ is \kl{ultimately polynomial} by \cite{CDTL23}.
...
\textbf{TODO: prove the following conjecture to conclude on polynomials in general
...
\begin{conjecture}
    ...
    \begin{itemize}
        \item Is $P$ \kl{represented} by a \kl{$\Rel$-polyregular function}?
        \item Is $P$ \kl{represented} by a \kl{$\Nat$-polyregular function}?
    \end{itemize}
\end{conjecture}
