...
\begin{proof}
    ...
    Now, consider an \kl{$\omega$-type} $t \in \ModuloTypes[\omega]^k$. By
...
\kl{$\Nat$-polyregular function}. Recall that the polynomial $P(X) \defined
X(X-1)/2$ can be computed by a \kl{$\Nat$-polyregular function}. Historically,
...
will actually be \kl{integer binomial polynomials} thanks to
