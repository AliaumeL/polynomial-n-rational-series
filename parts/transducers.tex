...
\begin{proof}
  ...
  is \kl{ultimately polynomial}.
  Let $q \colon \Nat^k \to \Sigma^*$ be a \kl{pumping pattern}.
  Because $(\Sigma^*, \resleqsf{f}{k})$ is an \kl[aperiodic
  ordering]{aperiodic} \kl{well-quasi-ordering}, there exists $N_0 \in \Nat$
  such that for all $x_1 \geq N_0$,
  $q(x_1, X_2, \dots, X_k) \resleqsf{f}{k} q(x_1 + 1, X_2, \dots, X_k)$.
  This proves that 
  $f(q(x_1 + 1, x_2, \dots, x_k)) - f(q(x_1, x_2, \dots, x_k))$
  is definable in $\NSF[k-1]$ as a function of $(x_2, \dots, x_k)$.
...
Similarly, if $f \in \ZSF[1]$ has non-negative values, then $f \in \NSF[1]$.
However, one cannot rely on the same argument, because we do not know whether
\cref{sf-no-periods-on-sequences:conj} holds or not. This is why the following
lemma requires an ad-hoc construction, based on the following fact: for every
function $g \in \ZSF[1]$ with non-negative values, the languages $\setof{u \in
\Sigma^*}{g(u) = c}$ are star-free for all $c \in \Nat$.

\begin{lemma}
  \label{linear-zsf-non-neg-languages:lem}
  Let $f \in \ZSF[1]$ be a function with non-negative values.
  Then, for every $c \in \Nat$,
  the language $f^{-1}(\set{c})$ is star-free.
\end{lemma}
\begin{proof}
  Let $f \in \ZSF[1]$ be a function with non-negative values.
  We want to prove that for every $c \in \Nat$,
  the language $f^{-1}(\set{c})$ is star-free.

  We base our proof on the same starting point as in \cref{linear-npoly-zpoly:lem}:
  given a function $f \in \ZSF[1]$ with non-negative values,
  we are given a finite \kl{aperiodic monoid} $M$,
  a surjective morphism $\mu \colon \Sigma^* \to M$,
  and a \kl{production function} $\pi \colon M^2 \to \Rel$.

  As in the proof of \cref{linear-npoly-zpoly:lem},
  we build a vector of functions $\llbracket u \rrbracket \colon M \to \Rel$ for each
  word $u \in \Sigma^*$ defined by
  \begin{equation*}
    \llbracket u \rrbracket(m)
    \defined
    \sum_{u = s_1 s_2}
    \pi(\mu(s_1), \mu(s_2) m) \quad .
  \end{equation*}

  Now, let us fix $c \in \Nat$.
  We can express the language $f^{-1}(\set{c})$ as follows:
  \begin{equation*}
    f^{-1}(\set{c})
    =
    \bigcup_{\substack{
      v \in M \\ 
      \llbracket v \rrbracket(m) + 
      \sum_{w = t_1 t_2} 
      \pi(\mu(v), \mu(t_1) m) = c
    }}
    \mu^{-1}(v) 
    ...
    \setof{w \in \Sigma^*}{
      \sum_{w = t_1 t_2} 
      \pi(\mu(v), \mu(t_1) m) = c - \llbracket v \rrbracket(m)
    } \quad .
  \end{equation*}
\end{proof}

\begin{lemma}
    \label{linear-zsf-nsf:lem}
    Let $f \in \ZSF[1]$ be a function with non-negative values.
    Then, $f \in \NSF[1]$.
\end{lemma}
\begin{proof}
  ...
  \begin{equation*}
    ...
    \pi(\mu(s_1), \mu(s_2) m) \quad .
  \end{equation*}
  And we know that these vectors are bounded below by some $-B$, where 
  $B \in \Nat$ is a constant independent of $u$.

  Not having access to \cref{sf-no-periods-on-sequences:conj}, or a way to
  build a \kl{residual transducer} for $f$, we will tweak the monoid $M$ and
  the production function $\pi$ to make it non-negative. To that end, let us
  first remark that for every $m \in M$, the function $u \mapsto \llbracket u
  \rrbracket(m)$ belongs to $\ZSF[1]$, and is bounded below by $-B$.

  We want to build an \kl{aperiodic monoid} $M'$, and a surjective morphism
  $\mu' \colon \Sigma^* \to M'$, such that for every $u \in \Sigma^*$,
  $\mu'(u)$ contains enough information to compute all the values of
  $\llbracket u \rrbracket(m)$ between $-B$ and $B$. This is possible because
  for every $m \in M$, for every $c \in \set{-B, \dots, B}$, the language
  $\setof{u \in \Sigma^*}{\llbracket u \rrbracket(m) = c}$ is star-free.

  We can now use $M \times M'$ as our new \kl{aperiodic monoid}, and define
  $\tau \colon \Sigma^* \to M \times M'$ as $\tau(u) = (\mu(u), \mu'(u))$.
  Finally, the production function will be as follows:
  we compute as $\pi$, but
  \begin{itemize}
    \item whenever the result is below $0$, we output $0$ instead.
    \item whenever the result is above $0$, if the value of $\llbracket u \rrbracket(m)$
      is negative equal to $y$, we only output $\pi - y$ (also capped to $0$).
  \end{itemize}
  This ensures that all the produced values are non-negative, and that 
  the monoid used is \kl(monoid){aperiodic}. A straightforward induction
  proves that this new function computes $f$.
\end{proof}
