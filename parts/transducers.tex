...
\reintro{good}. A sequence is \intro{bad} when it is not \reintro{good}. The
notion of \reintro{good sequences} can also be applied to binary relations that
are not orderings, and a binary relation $R$ for which every infinite sequence
is \reintro{good} is said to be \intro{well} \cite{MELL98}.
...
\intro{downwards closed} subset of $\Sigma^*$ for the prefix ordering
$(\prefleq)$, i.e. that every prefix of a state is also a state. 
...
We now introduce \cref{residual:algo} to
