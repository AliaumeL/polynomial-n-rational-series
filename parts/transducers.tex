...
\begin{example}
    ...
    The function is \kl{computed} by the two automata
...
\begin{figure}
    ...
    \kl{computing} the function of \cref{non-canonical-transd:ex}.}
    \label{non-canonical-transd:fig}
\end{figure}

\AP As in the case of finite automata, we generalize the function $\delta
...
= \aTransd(\delta(q, u), v) + \lambda(q, u)(v)$. Finally, let us notice that if
$\mathcal{H}$ is closed under finite sums and shifts, then $\lambda(q, u) \in
\mathcal{H}$ for all $(q,u) \in Q \times \Sigma^*$.
...
This notion can be applied to \kl{$\mathcal{H}$-transducers} by considering
...
\Rightarrow \resleq{f}{k})$ is \kl{well}, but the quas-ordering $(\resleq{f}{k})$ is a
