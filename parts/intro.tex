...
is also an open problem (see e.g. \cite{KARH77}), that remains open even in the
...
this class are actual polynomial functions, which were studied by Karhumäki in
\cite{KARH77}. Our first contribution is to provide a counter-example to the
main theorem of \cite{KARH77} that claimed to characterize the (multivariate)
polynomials in $\Rat[\vec{X}]$ that can be computed by some \kl{$\Nat$-rational
series} (\cref{thm:counter-example}). Our second contribution is to correct
this error, and provide (effective) characterizations of polynomials in
$\Rat[\vec{X}]$ that can be computed by \kl{$\Rel$-polyregular functions}
(\cref{integer-binomial-polynomials:cor}), and by \kl{$\Nat$-polyregular
functions} (\cref{decide-rat-poly-npoly:cor}). Our third contribution
