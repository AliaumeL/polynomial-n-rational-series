...
\begin{example}
	...
	The function $f \colon \set{a,b}^* \to \Nat$ defined by $f(w) \defined \card[a]{w} + 2\card[b]{w}$
	is \kl{commutative}. Indeed, for any permutation of the letters in $w$,
	the number of occurrences of $a$ and $b$ remains the same, hence $f$ is constant
	on permutations. For instance, $f(aab) = 1 + 2 \cdot 2 = 5 = f(aba) = f(baa)$.
	This function factors through the Parikh vector as $f = g \circ \commute$,
	where $g \colon \Nat^{\set{a,b}} \to \Nat$ is defined by $g(n_a, n_b) \defined n_a + 2n_b$.
\end{example}
...
preordering} of \kl{monomials}. In the polynomial $P(X,Y) \defined X^2 - 2XY +
Y^2 + X + Y$, the set of \reintro{maximal monomials} is $\set{X^2,  -2 XY,
Y^2}$. For instance, the \kl{non-negative} \kl{monomials} of $P(X,Y) \defined
...
\begin{example}
  \label{constant-function-nsf:ex}
  Let $c \in \Nat$, and 
  $f$ be the constant function $f \colon w \mapsto c$. 
  Then, $f \in \NSF[0]$.
\end{example}
\begin{proof}
  We consider the trivial monoid $M \defined (\set{1}, \times)$ (which is \kl(monoid){aperiodic})
  the morphism $\mu \colon \Sigma^* \to M$ defined by $\mu(w) \defined 1$,
  and the \kl{production function} $\pi \colon M \to \Nat$
  defined by $\pi(1) \defined c$.
\end{proof}
...
\end{example}
\begin{proof}
  Similarly to \cref{constant-function-nsf:ex},
