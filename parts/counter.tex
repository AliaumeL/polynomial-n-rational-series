...
polynomial}. The analogue notion of \intro{$\Rel$-rational polynomials} is not
of particular interest, since every polynomial $P$ is \kl{represented} by a
\kl{$\Rel$-polyregular function}. Our goal is to decide if a
\kl{$\Rel$-rational polynomial} (so, any polynomial) is an \kl{$\Nat$-rational
polynomial}, and if so, to compute an \kl{$\Nat$-polyregular function} that
\kl{represents} it. This particular problem was claimed to be solved in
\cite{KARH77}, using the \cref{karh:def} to characterize \kl{$\Nat$-polyregular
functions}, as restated in \cref{karh:thm}.
